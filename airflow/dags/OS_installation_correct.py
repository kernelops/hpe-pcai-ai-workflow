...
with DAG(
    ...
    doc_md="""
# HPE OS Software Simulation DAG

This DAG uses KVM/Libvirt on an remote Linux host to simulate the bare-metal provisioning capabilities of an HPE OS, performing the following strictly isolated operations:

1. Installs Hypervisor tools if missing (`qemu-kvm`, `libvirt`, `virtinst`).
2. Downloads a lightweight OS Image (Alpine Linux) simulating an OS Virtual Media image.
