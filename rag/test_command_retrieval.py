"""
test_command_retrieval.py

Direct test script for DAG command retrieval.

Flow:
1. Reads DAG source from `dag_source`
2. Extracts commands using dag_parser.py
3. Runs command RAG retrieval using run_dag_command_pipeline()
4. Prints final JSON response to terminal
...
try:
    ...
    from .rag_engine import run_dag_command_pipeline
...
# -------------------------------------------------------------------
# PASTE YOUR ENTIRE DAG SOURCE CODE BELOW
# -------------------------------------------------------------------
...
def main():
    ...
    print("[TEST] Extracting commands from DAG...")
    found_commands = extract_commands_from_dag(dag_source)

    print(f"[TEST] Found commands: {found_commands}")

    print("[TEST] Running DAG command pipeline...")
