...
def _require_nodes(**context) -> list[dict]:
    ...
    if len(nodes) < 1:
        raise RuntimeError("NFS inconsistency demo requires at least one reachable worker node")
    return nodes
...
def prepare_host_nfs_servers(**context):
    ...
    for index, node in enumerate(nodes[:2], start=1):
        ...
        command = (
            ...
            "  echo 'INFRASTRUCTURE ERROR: NFS packages (nfs-kernel-server, nfs-common) are not installed on this node.' >&2; "
            "  echo 'Run: sudo apt-get install -y nfs-kernel-server nfs-common' >&2; exit 1; "
            "fi; "
...
def mount_workers_to_nfs_a(**context):
    ...
    for node in nodes[:2]:
        command = (
            ...
            "  echo 'INFRASTRUCTURE ERROR: nfs-common package is not installed on this node.' >&2; "
            "  echo 'Run: sudo apt-get install -y nfs-common' >&2; exit 1; "
            "fi; "
...
def simulate_nfs_mount_inconsistency(**context):
    """Skipped in healthy DAG."""
    print("NFS drift injection skipped for healthy deployment.")
...
with DAG(
    dag_id="good_deployment_workflow",
    start_date=datetime(2024, 1, 1),
    schedule=None,
    catchup=False,
    tags=["deployment", "healthy"],
) as dag:
    ...
    simulate_os_validation_error = SSHOperator.partial(
        ...
        command=(
            "set -e; "
            "echo 'Simulating healthy OS validation...'; "
            "uname -a; "
            "echo 'OS baseline validation passed.'"
        ),
        get_pty=True,
        do_xcom_push=True,
    ...
    simulate_minio_service_error = SSHOperator.partial(
        ...
        command=(
            "set -e; "
            "echo 'Simulating healthy MinIO service check...'; "
            "echo 'MinIO service is running smoothly.'"
        ),
        get_pty=True,
        do_xcom_push=True,
    ).expand(ssh_conn_id=create_connections.output)
    ...
    simulate_postcheck_error = SSHOperator.partial(
        ...
        command=(
            "set -e; "
            "echo 'Simulating healthy post-deployment validation...'; "
