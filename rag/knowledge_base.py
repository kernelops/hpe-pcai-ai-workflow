...
def _lexical_score(query: str, doc_text: str, source: str) -> float:
    ...
    for phrase in important_phrases:
        if phrase in query_lower and phrase in doc_text_lower:
            score += 6.0

    if "minio" in query_lower and "minio" in source_lower:
        score += 5.0
    if "nfs" in query_lower and "nfs" in source_lower:
        score += 5.0
    if "os validation" in query_lower and "os validation" in source_lower:
        score += 5.0
    if "logs" in source_lower:
        score += 1.5
...
def retrieve_context(
    ...
    try:
        for collection_name in [DOCS_COLLECTION, ERRORS_COLLECTION]:
            ...
            for doc, meta in zip(query_results["documents"][0], query_results["metadatas"][0]):
                ...
                key = (source, doc)
                score = _lexical_score(query, doc, source) + 0.25
                existing = candidates.get(key)
                if not existing or score > existing["score"]:
                    candidates[key] = {"text": doc, "source": source, "score": score}
    except Exception as exc:
        print(f"[KnowledgeBase] Chroma query failed, using lexical fallback: {exc}")

    for item in MOCK_HPE_DOCS + MOCK_PAST_ERRORS:
        score = _lexical_score(query, item["text"], item["source"])
        if score > 0:
    ...
    return [{"text": item["text"], "source": item["source"]} for item in ranked[:limit]]
