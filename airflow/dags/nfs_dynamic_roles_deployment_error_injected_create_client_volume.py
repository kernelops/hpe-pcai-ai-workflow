"""
Error-injected DAG variant for dynamically distributing NFS Server and Client roles at runtime.
Injection point: create_client_volume.
"""
import os
import requests
...
with DAG(
    dag_id='nfs_dynamic_roles_deployment_error_injected_create_client_volume',
    default_args=default_args,
    description='Dynamically assign Server and Client roles for NFS deployment (error at create_client_volume)',
    schedule_interval=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['nfs', 'docker', 'ssh', 'dynamic', 'error-injected'],
) as dag:
...
docker volume create --driver local \\
  --opt type=nfs \\
  --opt o=addr=127.0.0.1,rw,nolock,nfsvers=3 \\
  --opt device=:/data \\
  nfs_client_vol
echo "INJECTED_ERROR: forcing failure in create_client_volume" >&2
exit 46
        '''
    )
...
alpine sleep 3600
    ...
    write_test_file = DynamicSSHOperator(
        ...
        cmd_timeout=30,
        command='''
docker exec nfs_client_test sh -c "echo \\\"Hello from the dynamically allocated client node!\\\" > /mnt/nfs/test_file.txt"
        '''
    )

    get_nodes >> create_connections >> assign_roles >> create_server_storage >> deploy_server
